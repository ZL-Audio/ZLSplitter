...
namespace zlp {
    ...
    void ControllerAttach<FloatType>::parameterChanged(const juce::String& parameter_ID, const float new_value) {
        if (parameter_ID == zlp::PSplitType::kID) {
            ...
            parameters_ref_.getParameterAsValue(zlp::PTSBalance::kID).setValue(zlp::PTSBalance::kDefaultV);
            parameters_ref_.getParameterAsValue(zlp::PPSBalance::kID).setValue(zlp::PPSBalance::kDefaultV);
