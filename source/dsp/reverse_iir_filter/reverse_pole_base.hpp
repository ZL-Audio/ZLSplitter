...
namespace zlReverseIIR {
    ...
    class ReversePoleBase {
        ...
        void prepare(const juce::dsp::ProcessSpec &spec) {
            ...
            for (auto &buffer : us) {
                buffer.setSize(1, static_cast<int>(spec.maximumBlockSize));
            }
        ...
        void process(juce::dsp::AudioBlock<SampleType> block) {
            for (size_t channel = 0; channel < static_cast<size_t>(block.getNumChannels()); ++channel) {
                juce::AudioBuffer<SampleType> &u(us[channel]), &v(vs[channel]);
                u.setSize(block.getNumSamples());
                v.setSize(block.getNumSamples());
                ...
                for (int index = 0; index < block.getNumSamples(); ++index) {
                    {
                        const auto uCurrent = u.getSample(0, index);
                        const auto vCurrent = v.getSample(0, index);
                        u.setSample(0, index,
                            as[0] * uCurrent - bs[0] * vCurrent + uDelay[0].popSample(0));
                        v.setSample(0, index,
                            bs[0] * uCurrent + as[0] * vCurrent);
                        uDelay[0].pushSample(0, uCurrent);
                    }
                    for (size_t stage = 1; stage <= numStage; ++stage) {
                        const auto uCurrent = u.getSample(0, index);
                        const auto vCurrent = v.getSample(0, index);
                        u.setSample(0, index,
                            as[stage] * uCurrent - bs[stage] * vCurrent + uDelay[stage].popSample(0));
                        v.setSample(0, index,
                            bs[stage] * uCurrent + as[stage] * vCurrent + vDelay[stage].popSample(0));
    ...
    private:
        ...
        std::vector<juce::AudioBuffer<SampleType>> us, vs;
    };
}
